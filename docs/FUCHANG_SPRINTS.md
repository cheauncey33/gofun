...
| Sprint | 主题 | 交付物 | 验收 |
|--------|------|--------|------|
| A | 产品可运营 | 下架/停售、批量退款、排队体验、边界整理 | 主办方可停售并退未核销票；用户看见排队；主路径统一 |
...
### A4 边界整理
- 删除未接线模块，并同步清理失效文档、脚本和构建产物引用
- 支付展示统一为“支付沙箱/待渠道回调”，平台不展示或维护外部用户余额
- container 去掉无引用的旧 repository 依赖
...
- 订单只保存支付结果，不读取或扣减用户余额；真实渠道还需查询、对账、重试和恢复

### C3 票码密钥轮换
- `ticket_qr.previous_secrets` 支持旧密钥并行验签，格式为 `FC1.<payload>.<signature>`
- 签发用当前密钥；校验遍历有效密钥
...
上表最小切片已落地；支付沙箱与基础电子票 E2E 已完成。真实渠道生产化、
支付重启恢复、支付并发边界和更完整的多票/并发核验验收仍未完成，详见
`docs/FUCHANG_COLLAB.md` 与 `docs/FUCHANG_PAYMENT_SANDBOX.md`。

在原 A/B/C 范围之后，当前代码还已落地：

- 无座票与在线选座并存，座位支持 `available / held / sold` 状态流转。
- 候补创建、支付窗口、顺序排队、释放票额派发和截止退款。
- 主办方申请与平台审核、活动送审/撤回/审核。
- 主办方按日期区间查看访问、下单、支付和核销漏斗。
- 管理员控制台与主办方控制台拆分。
