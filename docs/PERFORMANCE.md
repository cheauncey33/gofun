...
> 一页看清：赴场抢票链路的入口吞吐、库存热点优化，以及优化过程中的取舍。
> 原始数据见 `tests/load/results/`；复现命令见文末。

## TL;DR

- **入口合格峰值**：成功率 ≥99% 且 p99 ≤300ms 的 SLO 下，约 **1100~1450 req/s**（k6 恒定 VU，甜点区 100~200 VU）。
...
- 关键配置：Outbox `batch` 写入、库存分桶 8 桶、消费 worker 4×5
...
| **200** | **1442** | 205 | 238 | **312** | 138 | 100% | ✅ **甜点区** |
