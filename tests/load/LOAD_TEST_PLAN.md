...
1. Smoke test: validates the environment and one full user path.
2. Read baseline: measures read-only capacity.
3. Shopping mix: simulates normal users browsing and occasionally ordering.
4. Order write: focuses on Redis pre-deduct, RabbitMQ, and MySQL order creation.
5. Seckill spike: focuses on active seckill token and execute paths.
